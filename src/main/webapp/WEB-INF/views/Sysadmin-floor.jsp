			...
			<div class="col-sm-9 right-part">
				<div class="row">
					...
					<button class="btn btn-default" data-toggle="modal"
						data-target="#addbuilding">Add a Floor</button>
				</div>
				<div class="row">
					...
					<table class="table table-bordered table-striped table-hover">
						...
						<%-- <c:forEach var="i" begin="0" end="${sessionScope.floorlistsize}"
							step="1">

							<tr>
								<td><c:out value="${sessionScope.floorlist[i]}"></c:out></td>
							</tr>
						</c:forEach> --%>
						<c:forEach var="floor" items="${sessionScope.floorlist}">
							<tr>
								...
								<td>${floor.buildingname.enterprisename}</td>
								<td>${floor.buildingname.enterprisename.cityname}</td>
							</tr>
						</c:forEach>
					</table>
				</div>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h3 class="modal-title">Add a floor</h3>
				</div>
				<div class="modal-body">

					<form action="addfloor.htm" method="post" id="addfloorform">
						<div class="form-group">
							<select class="form-control" name="buildingselect"
								id="buildingselect">
								<c:forEach var="building" items="${sessionScope.buildinglist}">
									<option value="${building.buildingname}">${building.enterprisename.cityname}
										> ${building.enterprisename} > ${building.buildingname}</option>
								</c:forEach>
							</select>
							<div class="form-group">
								<label for="name">Floor Name</label> <input type="text"
									name="floorname" id="floorname-text" class="form-control"
									required />
							</div>
							<div class="errormsg">This floor has been added</div>
							<button type="submit" id="addfloorbtn" class="btn btn-default">Add</button>
						</div>
					</form>
		...
		<div class="modal-dialog">
			<div class="modal-content action-success">
				...
				<div class="modal-header">
					...
					<h3 class="modal-title">Create Floor Succefully!</h3>
				</div>
